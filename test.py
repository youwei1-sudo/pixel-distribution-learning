    ...
    with torch.no_grad():
        for i in range(len(imgs)):
            ...
            for val in range(value):
                select_patch = flow_patch_list[val * batch_size: (val + 1) * batch_size].transpose(0, 2, 3, 1)
                select_patch_large = flow_patch_list_large[val * batch_size: (val + 1) * batch_size].transpose(0, 2, 3, 1)
                ...
                select_pixels = select_batch_size_patch(random_patch_list, patch_size, channel, batch_size,
                                                        select_pixels_size)
                select_pixels_large = select_batch_size_patch(random_patch_large_list, patch_size_larger, channel,
                ...
                np_select_pixel = np.concatenate((np_random_select_pixel_list, np_random_select_pixel_list_large),
                                                        axis=3)

                # reshape
                np_select_pixel = np_select_pixel.transpose(0, 3, 1, 2)
                ...
                input_patch = torch.FloatTensor(np_select_pixel)
